...
class Exp_PointSeg:
    ...
    def train(self, setting):
        ...
        if self._seg_loss_cfg()["class_weight"] == "auto":
            ...
            # Effective Number Reweighting: weight = (1 - beta^N) / (1 - beta), beta=0.9999
            # 对少数类施加指数级更大的权重
            # simpler ratio-based weighting
            neg = cnt.get(0, 0)
            pos = cnt.get(1, 0)
            ratio = neg / pos if pos > 0 else 1.0
            class_weights = [1.0, float(ratio)]
            # apply extra seg_pos_weight if given
            ...
            _print("Class weights (ratio): {}".format([f"{w:.2f}" for w in class_weights]), self.path)
        else:
        ...
        best_epoch = -1
        best_info = {}
        early_stop_patience = int(getattr(self.args, "early_stop_patience", 10))
        ...
        for epoch in range(self.args.train_epochs):
            ...
            # 正类检测能力下界检查：如果recall_cls1 < 0.1则告警
            recall_cls1_guard = float(getattr(self.args, "pointseg_recall_cls1_guard", 0.05))
            if recall_cls1_guard > 0 and val_pred_pos_rate > 0:
                # 粗估recall：pred_cls1 / gt_cls1 (注意：这是个近似)
                if val_gt_pos_rate > 0.001:  # 避免除以0
                    est_recall = (val_pred_pos_rate * 0.5) / val_gt_pos_rate if val_pred_pos_rate > 0 else 0
                    if est_recall < recall_cls1_guard:
                        _print("WARNING epoch {}: Est. recall_cls1 {:.4f} < guard {:.4f}, pred_cls1={:.4f} gt_cls1={:.4f}".format(
                            epoch + 1, est_recall, recall_cls1_guard, val_pred_pos_rate, val_gt_pos_rate), self.path)
            ...
            if (current_best_metric >= best_val_metric) and pass_pos_guard:
                ...
                if early_stop_counter >= early_stop_patience and early_stop_patience > 0:
                    _print("Early stopping triggered at epoch {} after {} epochs without improvement".format(
                        epoch + 1, early_stop_patience), self.path)
                    break
            
            if (current_best_metric >= best_val_metric) and (not pass_pos_guard):
