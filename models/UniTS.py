...
class PatchEmbedding(nn.Module):
    def __init__(self, d_model, patch_len, stride, padding, dropout):
        ...
        self.stride = stride
        # 允许 stride < patch_len（重叠）；point_seg 时需 patch_len == stride 或 SegHead 做重叠聚合
...
class SegHead(nn.Module):
    """Point-wise segmentation head: token sequence [B,V,L,D] -> logits [B, num_classes, T]. Supports stride < patch_len (overlap) by averaging."""

    def __init__(self, d_model, num_classes, patch_len, stride=None, head_dropout=0):
        super().__init__()
    ...
    def _fold_overlap(self, x, seq_len):
        """Fold [B, C, L*patch_len] to [B, C, seq_len] by averaging overlapping patch predictions."""
        B, C, _ = x.shape
        L = x.shape[2] // self.patch_len
        out = torch.zeros(B, C, seq_len, device=x.device, dtype=x.dtype)
        cnt = torch.zeros(1, 1, seq_len, device=x.device, dtype=x.dtype)
        for i in range(L):
            start = i * self.stride
            end = min(seq_len, start + self.patch_len)
            if start < seq_len and end > start:
                n = end - start
                out[:, :, start:end] += x[:, :, i * self.patch_len : i * self.patch_len + n]
                cnt[:, :, start:end] += 1
    ...
    def forward(self, x_token, seq_len):
        ...
        if x.shape[-1] > seq_len:
            if self.stride < self.patch_len:
                x = self._fold_overlap(x, seq_len)
            else:
                x = x[:, :, :seq_len]
    ...
    def __init__(self, args, configs_list, pretrain=False):
        ...
        for i in range(self.num_task):
            ...
            if configs_list[i][1]['task_name'] == 'classification':
                ...
                self.seg_heads[task_data_name] = SegHead(args.d_model, num_classes, patch_len=args.patch_len, stride=getattr(args, 'stride', args.patch_len), head_dropout=args.dropout)
